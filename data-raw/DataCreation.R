...
devtools::use_data(rfidata, overwrite = T)

RFIanalysis <- readRDS("simulation/DataAnalysis/RFIanalysis.rds")
res <- RFIanalysis$CAR1
res$TimeMinOut <- NULL
res$ori.res$newlmSymm <- NULL
sampleid <- sample(nrow(dat), 1000)
dat <- dat[sampleid,]
res$ori.res$v$E <- round(res$ori.res$v$E[sampleid,], 3)
res$ori.res$v$weights <- round(res$ori.res$v$weights[sampleid,],3)
res$pqvalue$pv <- round(res$pqvalue$pv[sampleid,],3)
res$pqvalue$qv <- round(res$pqvalue$qv[sampleid,],2)
res$ori.res$newlm <- round(res$ori.res$newlm[sampleid,],3)

covset$time <- res$NewTime

devtools::use_data(dat, covset, design,res, overwrite = T)
...
res$ori.res$newlm <- round(res$ori.res$newlm[sampleid,],3)
resSymm <- res
devtools::use_data(resSymm, overwrite = T)
